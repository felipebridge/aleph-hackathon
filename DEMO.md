...
Si preguntan "¿lo probaron con el worker real corriendo?": ser honestos — se validó la integración contra la API real del SDK instalado (`Client`, `load_model`, `completion`, el schema de `attachments`), pero la demo en vivo corre con el motor de respaldo porque el worker de QVAC no estaba disponible en la laptop de desarrollo. El diseño con motores intercambiables (`--ocr-engine qvac|tesseract|mock`) es justamente para que eso no bloquee ni el desarrollo ni la demo.
